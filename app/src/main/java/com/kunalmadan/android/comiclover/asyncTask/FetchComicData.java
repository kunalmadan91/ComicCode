...
public class FetchComicData extends AsyncTask<String, Void, ArrayList<Comic>> {
    ...
    public static ArrayList<Comic> all;
    ...
    @Override
    protected void onPostExecute(ArrayList<Comic> data) {
        super.onPostExecute(data);
        progressDialog.dismiss();
        Log.v("kmkm","kmkm"+data.get(0));
       // EventBus myEventBus = EventBus.getDefault();
        EventBus.getDefault().post(new DataEvent(data));

       // EventBus.getDefault().post(new Comic(data.getTitle(),data.getDescription(),data.getPrice(),data.getThumbnnail(),data.getOnSaleDate()));
